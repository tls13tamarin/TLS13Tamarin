...
#define St_init_0rtt(X,Y,...) St_##X##_##Y##_init_0rtt(__VA_ARGS__) 
#define St_resume_0rtt(X,Y,...) St_##X##_##Y##_resume_0rtt(__VA_ARGS__) 
#define St_loop_0rtt(X,Y,...) St_##X##_##Y##_loop_0rtt(__VA_ARGS__) 
...
lemma pms_invariant [use_induction,reuse]:
  "All tid actor peer es ss #i. LoopSecrets(tid,actor,peer,es,ss)@i==>
          (Ex #j. Secrets(tid,actor,peer,es,ss)@j & #j<#i)"

lemma inittid_before_loop [use_induction,reuse]:
  "All tid sid role #i. Loop(tid,sid,role)@i==>Ex #j. InitTid(tid,sid,role)@j & #j<#i" 

lemma es_secret [reuse]:
  "All tid actor peer es ss #i #j #k. Secrets(tid,actor,peer,es,ss)@i & KU(es)@j & Secret(tid)@k==>
//#r<#i, not #r<#j because attacker needs key for signing
         (Ex #r. RevLtk(peer)@r & #r<#i)
         |(Ex #r. RevDHExp(tid)@r)
         |(Ex tid2 #r #s. RevDHExp(tid2) @ r &
          Partner(tid2,tid,peer,actor) @ s)"

lemma ss_secret [reuse]:
  "All tid actor peer es ss #i #j #k. Secrets(tid,actor,peer,es,ss)@i & KU(ss)@j & Secret(tid)@k==>
//#r<#i, not #r<#j because attacker needs key for signing
         (Ex #r. RevLtk(peer)@r & #r<#i)
         |(Ex #r. RevDHExp(tid)@r)
         |(Ex tid2 #r #s. RevDHExp(tid2) @ r &
...
lemma secret_sessionkeys [use_induction]:
   "All tid actor peer key #i #j.
          SessionKey(tid, actor, peer, key) @ i
        & KU(key) @ j
        ==>
#ifdef PFS
       	(Ex #r. RevLtk(peer)@r & #r<#i)
